...
- [ ] **문서 파서 — 일반 PDF 라이브러리 금지.** CLAUDE.md "문서 원본 포맷" 참고:
      `.pdf` 확장자이나 실제로는 zip(페이지별 JPEG + OCR 텍스트). `zipfile.is_zipfile()`로
      먼저 판별하는 커스텀 파서부터 작성할 것. DOCX/XLSX는 표준 라이브러리로 처리 가능
- [ ] 표 구조를 깨뜨리지 말 것 (세율 대조에 필요)
...
- [ ] `build_answer()`에 7종 주입 완료 (신규 5종 + verify_grounding/generate_answer 연결 2종)

...
# ════════════════════════════════════════════════════════════════
# F42 · 상품 비교 질의(대주제 2)가 UNRELATED_TOPIC에 통째로 막혔다
# ════════════════════════════════════════════════════════════════
#
# 실측(UI-022, 2026-09-06) — 과제 안내 4페이지의 예시 질의 그대로
# "솔로몬 국공채 단기·중장기·장기, 뭐가 달라요? 안정적인 걸 원해요"가
# UNRELATED_TOPIC으로 거절됐다. DOMAIN_TERMS(vocab.py, 제도 용어 위주:
# 세액공제·IRP·연금저축 등)와 _FINANCE_CONTEXT_EXTRA(노후·은퇴·자산 등)
# 어디에도 "국공채"·"안정적" 같은 상품 어휘가 없어서다.
#
# 정작 `_DOMAIN_KEYWORDS`(DOMAIN_AREAS 전체를 모은 집합 — "국공채"가
# DOMAIN_AREAS["상품"]에 이미 있다)는 이 파일에 **정의는 돼 있었지만
# check_refusal의 조건문에서 빠져 있었다** — L0가 이 질의를 실제로
# '상품' 영역으로 정확히 분류하는데도, 거절 게이트는 그 분류 결과를 전혀
# 참고하지 않고 있었다. 결과적으로 위험등급·총보수를 색인 시점에 이미
# 뽑아 둔 상품 팩트 DB가 있어도, 상품 비교(대주제 2) 질의 자체가 이
# 문 앞에서 막혀 그 데이터에 도달하지도 못했다.

def test_상품_어휘만_있어도_거절하지_않는다():
    """★ 실측 재현 — 과제 안내 4페이지 예시 질의 그대로."""
    q = "솔로몬 국공채 단기 · 중장기 · 장기, 뭐가 달라요? 안정적인 걸 원해요."
    r = check_refusal(q)
    assert not r.refuse, f"상품 비교 질의가 잘못 거절됨: {r.code}"


@pytest.mark.parametrize("q", [
    "이 펀드 위험등급이 몇 등급인가요?",
    "채권형이랑 주식형 중에 뭐가 더 안전해요?",
    "총보수가 낮은 클래스는 뭔가요?",
])
def test_상품_비교_질의들이_거절되지_않는다(q):
    """대조군 — DOMAIN_AREAS['상품'] 어휘가 하나라도 있으면 통과해야 한다."""
    r = check_refusal(q)
    assert not r.refuse, f"{q} → {r.code}"


def test_DOMAIN_KEYWORDS가_실제로_참조된다():
    """★ 회귀 방지 — 정의만 되고 조건문에서 빠지는 '죽은 집합'이 재발하면 안 된다.

    이번 결함의 본질은 새 어휘가 없어서가 아니라, 있는 어휘 집합을
    조건문에 연결하는 걸 빠뜨린 것이었다. check_refusal 함수 본문
    소스에 이 이름이 실제로 나타나는지 직접 대조한다.
    """
    import inspect

    from app.analysis import refusal as refusal_mod

    src = inspect.getsource(refusal_mod.check_refusal)
    assert "_DOMAIN_KEYWORDS" in src

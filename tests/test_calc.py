...
# ════════════════════════════════════════════════════════════════
# F43 · 수리팀 새 산식(분모 max(1,·)) 통합 — 11년차 이상은 doc39 유지
# ════════════════════════════════════════════════════════════════
#
# 2026-09-06 — 수리 담당자가 "연금수령한도 = 1.2 × 평가액 /
# max(1, 11−연금수령연차)"라는 산식을 보내왔다. 1~10년차 구간에서는
# 11−연차가 항상 1 이상이라 기존 식과 수학적으로 완전히 같은 값을 낸다
# (아래 test_연금수령한도_원문예시가 이미 그 구간을 고정한다).
#
# 그런데 11년차 이상에서는 두 식이 갈린다 — max(1,·)로 분모 바닥을 깔면
# "평가액×1.2"라는 유한한 값이 나오는데, doc39 원문은 "11년차부터는 한도
# 자체가 사라져 전액 일시 인출을 하여도 연금수령으로 인정받을 수 있다"고
# 명시한다(유한한 상한이 아니라 무제한). 제공 문서가 최종 근거이고 외부
# 산식은 보조라는 원칙(CLAUDE.md)에 따라 사용자에게 확인한 결과, 11년차
# 이상은 기존 '무제한' 처리를 그대로 유지하기로 했다 — 위
# test_11년차부터는_한도가_없다가 그 불변식을 고정한다. 이 테스트는
# 새 산식이 "1~10년차 구간에서만" 반영됐음을 별도로 확인한다.

def test_새_산식의_분모_형태가_1_10년차_구간에_반영된다():
    """★ 배선 — denominator 필드가 max(1, 11-연차) 형태를 그대로 낸다."""
    for year in range(1, 11):
        out = calc_withdrawal_limit(10000, year)
        assert out["denominator"] == max(1, 11 - year)


def test_11년차_이상은_새_산식으로_유한값을_내지_않는다():
    """★ 핵심 불변식 — max(1,·) 산식을 11년차 이상에 그대로 적용하면
    안 된다는 사용자 결정을 고정한다. 어겼다면 limit이 유한한
    account_value*1.2가 됐을 것이다.
    """
    for year in (11, 15, 30):
        out = calc_withdrawal_limit(10000, year)
        assert out["limit"] is None
        assert out["unlimited"] is True

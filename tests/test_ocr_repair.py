...
# ── 3+런 복원 뒤 2글자 잔여 손상 마스킹 (2026-09-05 실측) ───────
#
# 서버 실측에서 runs_masked(5,189)가 (runs_found - runs_repaired = 5,256)
# 보다 67건 적었다 — 집계상 "복원도 안 됐는데 격리도 안 된" 손상이 있다는
# 뜻이다. 원인: 한 페이지 안에 STRONG(3+) 런과 WEAK(2글자) 런이 함께 있을
# 때, STRONG 런만 복원되고 나면 남은 텍스트에는 3+ 패턴이 더 이상 없다.
# `mask_unreadable`이 `looks_garbled`(3+ 기준)로 다시 게이팅하므로, 이미
# "오염 확인됨"으로 `garbled` 목록에 들어왔던 페이지인데도 마스킹을 통째로
# 건너뛰어 미복원 2글자 런이 원문 그대로 남는다 — 이 모듈이 막으려던
# "?????????가 그대로 청크에 실린다"와 같은 사고다.

def test_강한_런이_복원되고_약한_런만_남아도_마스킹된다():
    """★ 이 결함의 핵심 재현.

    '기기기'(3+, STRONG)는 다른 문서의 사본으로 복원되고, 그 뒤에 남는
    '여여'(2글자, WEAK)는 복원되지 않는다. 복원 후 텍스트에는 더 이상
    3+ 패턴이 없어 looks_garbled()가 False를 반환하지만, '여여'는 여전히
    격리(판독불가) 대상이어야 한다 — 원문 그대로 새어 나가면 안 된다.
    """
    broken = ("정상시작문맥이충분히깁니다감가AAAAAAAAAAAA기기기BBBBBBBBBBBB"
             "여여또다른충분히긴정상문맥으로끝납니다")
    twin = ("정상시작문맥이충분히깁니다감가AAAAAAAAAAAA정상값BBBBBBBBBBBB"
           "YYYYYYYYYYYY또다른충분히긴정상문맥")
    docs = [_doc("broken", broken), _doc("twin", twin)]
    report = repair_documents(docs)

    assert report.runs_repaired >= 1, "3+런('기기기')조차 복원되지 않았다 — 재현 조건 확인 필요"
    text = docs[0].pages[0].text
    assert "여여" not in text, f"미복원 2글자 런이 격리되지 않고 그대로 남았다: {text!r}"
    assert "기기기" not in text
    assert "정상값" in text, "3+런 복원 결과 자체가 반영되지 않았다"

...
# ★ 실서버 재인덱싱본에서 실제로 잡힌 오탐 (2026-09-04).
# doc_meta에 저장된 return_table이 위험등급 경고문 전체를 그대로 담고
# "과거 운용실적 표"라는 라벨을 달고 있었다 — 표가 아닌데 표로 인용됨.
_RISK_WARNING_NOT_A_TABLE = (
    "하여 1등급으로 분류하였습니다. 펀드의 위험 등급은 운용실적, 시장 "
    "상황 등에 따라 변경될 수 있다는 점을 유의하여 투자판단을 하시기 "
    "바랍니다.")


def test_위험등급_경고문을_실적표로_오인하지_않는다():
    """★ '운용실적'이라는 단어만으로 표를 판정하면 안 된다.

    표준 위험등급 경고문에 '운용실적'이라는 말이 흔히 들어가는데, 이게
    표 헤더로 오인되면 무관한 문장이 '과거 운용실적 표'라는 라벨을 달고
    답변 프롬프트에 실린다.
    """
    f = extract_product_facts(_RISK_WARNING_NOT_A_TABLE, "x")
    assert f.return_table is None, (
        f"위험등급 경고문을 실적표로 오인했다: {f.return_table}")

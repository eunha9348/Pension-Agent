...
def test_거절응답이_bridge_근거를_인용한다():
    """근거 없이 상품을 언급하면 '근거 없는 추천'이 된다."""
    from app.core.coverage_pipeline import TraceLogger
    from app.pipeline import _refuse_response

    store = _Store(["doc10", "doc11"])
    b = find_bridge("오늘 코스피 지수 알려줘", _Refusal(), store)
    out = _refuse_response("Q-1", "오늘 코스피 지수 알려줘", _Refusal(), [],
                           TraceLogger(), bridge=b)
    assert "doc10" in out["retrieved_context"]
    assert "근거 문서 없음" not in out["retrieved_context"]
    assert "거절_대신_연결" in out["think_trace"]


def test_다섯_필드가_모두_채워진다():
    """평가 API 스펙 — 어느 경로로 가든 5개 필드가 비면 안 된다."""
    from app.core.coverage_pipeline import TraceLogger
    from app.pipeline import _refuse_response

    store = _Store(["doc10", "doc11"])
    b = find_bridge("오늘 코스피 지수 알려줘", _Refusal(), store)
    out = _refuse_response("Q-1", "질문", _Refusal(), [], TraceLogger(), bridge=b)

...
# ═══════════════════════════════════════════════════════════════════
# F47(b) · 계좌 이름을 '언급'만 해도 보유로 보던 결함 (2026-09-06)
#
# conditions._ACCOUNT_SIGNALS는 "연금저축"이라는 단어가 나오기만 하면
# account_type을 채우는데, classify_route는 그 키의 존재만으로 GENERAL을
# 확정했다. "30대 초반인데 연금저축 지금 시작하는 게 좋을까요"가 계산할
# 것이 하나도 없는데 GENERAL로 가 "근거를 확인하지 못했습니다 + 문서
# 3건 나열"이 나갔다.
#
# ⚠️ account_type 추출 자체는 좁히지 않는다 — 자격 판정·상품 매칭 등이
#    쓰는 키라 파급이 크다. 라우팅에서만 걸러낸다.
# ═══════════════════════════════════════════════════════════════════


# ⚠️ 이 파일 맨 위의 _route()를 재정의하지 말 것 — 그것은 rule_based_spec을
#    거쳐 RouteDecision을 돌려주고 20건 넘는 기존 테스트가 그 객체의
#    .is_advisory를 읽는다. 이름을 겹치면 그 테스트들이 통째로 깨진다
#    (실제로 이 절을 추가하다 20건이 깨졌다). 여기서는 별도 이름을 쓴다.
def _route_name(q: str) -> str:
    return classify_route(q, derive_conditions(q), []).route


@pytest.mark.parametrize("q", [
    "30대 초반인데 연금저축 지금 시작하는 게 좋을까요 아니면 나중에 할까요",
    "IRP 아직 없는데 노후 준비 어떻게 해야 할까요",
    "연금저축 새로 만들까 하는데 어디부터 해야 하나요",
])
def test_아직_없는_계좌는_보유로_보지_않는다(q):
    assert _route_name(q) == "ADVISORY"


@pytest.mark.parametrize("q", [
    # 계좌가 아직 없어도 상품 어휘가 있으면 총보수 비교·판매클래스 자격
    # 판정이 돌아야 한다 — 대주제 2(상품 설명·비교)가 통째로 죽는다.
    "IRP 가입하려는데 총보수 낮은 클래스가 뭔가요",
    "연금저축 시작하려는데 어떤 상품이 좋을까요",
    "연금저축 새로 들려는데 위험등급 낮은 펀드 추천해주세요",
    # 보유·계산 조건이 분명한 기존 경로는 그대로 GENERAL이어야 한다
    "제 IRP는 어떤 클래스로 가입 가능한가요",
    "IRP 계좌에 1억 있는데 인출한도가 얼마인가요",
    "연금저축이랑 IRP에 넣으면 세액공제 얼마까지 되나요",
])
def test_대조군_상품맥락과_보유계좌는_GENERAL을_유지한다(q):
    assert _route_name(q) == "GENERAL"

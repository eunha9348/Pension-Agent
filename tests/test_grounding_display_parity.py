...
def test_억단위_분리표기가_근거없는_수치로_잡히지_않는다():
    ...
    format_manwon은 10,000만원 이상을 "1억 416.7만원"처럼 억과 만원으로
    쪼개 쓴다(⚠️ 정수로 반올림하지 않고 소수 첫째 자리까지 표시 —
    같은 날 반올림 폭 자체를 줄이기로 했다). 그런데 검증 대조 집합에
    반올림 보정값만 있으면, 답변에 실제로 찍히는 416.7이 '근거 없는
    수치'로 잡힌다. 그 결과 **계산이 정확히 성공했는데도** 답변이
    통째로 템플릿으로 축퇴했다.
    ...
    assert format_manwon(10416.6667) == "1억 416.7만원"
    assert 416.7 in allowed, "표시형의 만원 부분이 허용 집합에 없다"

...
def test_금지된_조문은_어떤_규칙에도_달리지_않는다():
    """수치가 같아 섞이기 쉬운 조문들 — 실수집본에서 실제로 확인된 것들이다.

      · 소득세법 제47조의2 제1항 : 900만원이지만 '연금소득공제' 한도로,
        C4의 '연금계좌세액공제' 900만원과 제도가 다르다.
      · 조세특례제한법 제86조의4 : 600/900을 담지만 2022.12.31 일몰된
        50세 이상 특례다. 현행으로 인용되면 사용자를 오도한다.

    새로 발견되는 혼동 조문은 BLOCKED_ANCHORS에 추가하면 여기서 함께 막힌다.
    """
    from app.law.anchors import ANCHORS, BLOCKED_ANCHORS

    for tid, refs in ANCHORS.items():
        for ref in refs:
            assert ref not in BLOCKED_ANCHORS, (
                f"{tid}에 금지 조문이 달렸습니다: {ref} "
                f"({BLOCKED_ANCHORS[ref]})")


def test_세액공제_규칙은_수치가_든_조문에_달린다():
    """C4·C5의 요점은 600/900만원이다.

    제59조의3 **제2항**은 공제의 '명칭'만 정의하고 수치가 없다. 수치 없는
    조문을 달면 HCX가 한도를 뒷받침하지 못하는 조문을 인용하게 된다.
    수치가 실린 것은 **제1항**이다(실수집본 --grep 으로 확인).
    """
    from app.law.anchors import ANCHORS

    for tid in ("C4", "C5"):
        refs = ANCHORS.get(tid, ())
        assert refs, f"{tid}에 앵커가 없습니다"
        assert "소득세법 제59조의3 제2항" not in refs, (
            f"{tid}에 수치 없는 조문(제2항)이 달렸습니다")
        assert "소득세법 제59조의3 제1항" in refs, (
            f"{tid}에 한도 수치 조문(제1항)이 없습니다")

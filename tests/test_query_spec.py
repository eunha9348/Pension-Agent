...
# ════════════════════════════════════════════════════════════════
# F56 · 위험자산 편입비율 질의가 주제 미매칭 → 근거 무관 → 절차 단정
#        3단 실패로 이어지던 결함 (2026-09-07 실측, UI-003)
# ════════════════════════════════════════════════════════════════
#
# "개인이 직접 운용하는 방식으로 초기에 위험자산 비중을 70% 이하로
# 가져갔지만, 위험자산의 가격 급상승으로 비중이 70%가 넘게되면 어떻게
# 되나요?"에 주제 미매칭 → 검색이 무관한 문서 7건 인용 → HCX가 근거
# 어디에도 없는 "자동으로 리밸런싱이 진행됩니다"를 단정했다.
#
# fact-only TopicRule(계산함수 없음)을 신설해 요구사항 슬롯을 만든다 —
# 이것만으로 검색 랭킹이 개선된다는 보장은 없지만(실물 코퍼스 확인
# 불가), 최소한 "이 사실을 찾아야 한다"는 요구사항이 명시적으로
# 남아 커버리지 체크·한계 고지 경로를 태운다.

def test_위험자산_편입비율_질의가_주제로_매칭된다():
    spec = rule_based_spec(
        "개인이 직접 운용하는 방식으로 초기에 위험자산 비중을 70% "
        "이하로 가져갔지만, 위험자산의 가격 급상승으로 비중이 70%가 "
        "넘게되면 어떻게 되나요?")
    assert spec.get("intent") == "위험자산_편입비율"


def test_위험자산_투자한도_질의가_연금수령한도로_오분류되지_않는다():
    """★ replay 300건 G02 재현 — 바로 앞 규칙의 넓은 '한도' 키워드가
    엉뚱하게 계좌평가액·연금수령연차를 요구하던 결함(정답은 '위험자산
    70% 한도'인데 완전히 다른 계산으로 샜다)."""
    spec = rule_based_spec("IRP에서 위험자산 투자한도가 있나요?")
    assert spec.get("intent") == "위험자산_편입비율"
    fns = [c["function"] for c in spec["planned_calls"]]
    assert "연금수령한도_계산" not in fns

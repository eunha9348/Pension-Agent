...
class TopicRule:
    ...
    # "뭐가 달라"·"차이"처럼 **비교 자체를 뜻하는 말**은 상품 비교에도
    # 제도 비교에도 똑같이 쓰인다. 배제어로 제도 쪽을 하나씩 빼려 하면
    # 목록이 끝없이 길어지고, 빠진 표현 하나마다 제도 질의가 상품 비교로
    # 샌다("DC와 DB, 퇴직금이 정해지는 방식이 어떻게 다른가요?" — 안내서
    # 난이도 '하'의 제도 질의다).
    # 반대로 **무엇을 비교하는가**(상품 차원인가)는 긍정 조건으로 쓰면
    # 한 줄로 갈린다. 그래서 배제가 아니라 요구로 건다.
    require_any: tuple[str, ...] = ()


# 이 질의가 **상품 차원**인가를 가르는 어휘 (F49).
#
# ⚠️ 새 목록을 만들지 않고 L0의 DOMAIN_AREAS["상품"]을 그대로 재사용한다 —
#    같은 판단을 두 곳이 다른 기준으로 하면 반드시 어긋난다
#    (CLAUDE.md "인용 기준과 매칭 기준을 어긋나게 두지 말 것").
#    L0가 "솔로몬 국공채 …"를 영역 ['상품']으로 정확히 분류하는데 주제
#    규칙만 그 사실을 모르는 상태였던 것이 F49의 본질이다.
# ⚠️ 지연 import로 순환 참조를 피한다 — grounding_retrieval은 분석 계층을
#    참조하지 않지만, 모듈 최상단에서 묶으면 import 순서에 묶인다.
def _product_dimension_terms() -> tuple[str, ...]:
    from app.core.grounding_retrieval import DOMAIN_AREAS
    # 안내서 5페이지가 실적배당형을 "펀드 · ETF"로 정의하므로 ETF·TDF를
    # 함께 본다(DOMAIN_AREAS는 거절 게이트도 쓰는 공용 목록이라 그쪽 동작을
    # 바꾸지 않도록 여기서만 더한다).
    return tuple(DOMAIN_AREAS["상품"]) + ("ETF", "etf", "TDF", "tdf",
                                          "인덱스", "상품")


_PRODUCT_DIMENSION: tuple[str, ...] = _product_dimension_terms()
...
TOPIC_RULES: list[TopicRule] = [
    ...
    # ⚠️ "인출가능한 최대 금액" 류 자연어가 아래 키워드 어디에도 안 걸렸다
    #    (2026-09-07 실측, UI-006). "25세부터 30년간 IRP에 연 300만원씩
    #    납입했고 지금 55세인데, 연금수령으로 인출가능한 최대 금액이
    #    얼마인가요"가 주제 미매칭으로 떨어져 슬롯이 안 만들어졌고, HCX가
    #    일반 지식에서 **1,500만원 분리과세 기준**(과세방식이 갈리는 문턱일
    #    뿐 인출 한도가 아니다)을 "최대한도로 인출 가능한 금액"이라고
    #    잘못 답했다 — F36/F41/F44/F49와 같은 narrow-trigger 계열이다.
    #    "얼마까지 인출"·"얼마나 인출"은 '인출'이 '얼마' **뒤**에 오는
    #    어순만 잡고, "인출가능한 최대"·"받을 수 있는 최대"처럼 '인출/수령'이
    #    '최대' **앞**에 오는 흔한 어순은 못 잡는다.
    # ── 위험자산 편입비율 (F56 · 2026-09-07 실측, UI-003) ──────────
    # ⚠️ 계산함수·번역이 없는 **fact-only** 규칙이다. "개인이 직접
    #    운용하는 방식으로 초기에 위험자산 비중을 70% 이하로 가져갔지만,
    #    가격 급상승으로 비중이 70%가 넘게되면 어떻게 되나요?"가 주제
    #    미매칭으로 떨어져 검색이 무관한 문서 7건을 가져왔고, HCX가
    #    근거 어디에도 없는 "자동으로 리밸런싱이 진행됩니다"를 단정했다
    #    (verify_mechanism_claim_grounding이 그 답변을 잡지만, 애초에
    #    올바른 근거를 찾도록 요구사항 슬롯을 만드는 것이 먼저다).
    # ⚠️ replay 300건에도 같은 유형이 있다 — G02
    #    "IRP에서 위험자산 투자한도가 있나요?"가 아래 "연금수령한도"의
    #    **바로 그 넓은 "한도" 키워드**에 걸려 엉뚱하게 계좌평가액·
    #    연금수령연차를 요구했다(정답은 "위험자산 70% 한도"인데 완전히
    #    다른 계산으로 샜다). 이 규칙을 앞에 두고 "연금수령한도"에
    #    `exclude=("위험자산",)`을 추가해 서로 겹치지 않게 한다.
    TopicRule("위험자산_편입비율",
              ("위험자산 비중", "위험자산비중", "위험자산 편입비율",
               "위험자산편입비율", "위험자산 투자한도", "위험자산투자한도",
               "위험자산 한도", "위험자산한도"),
              "wiheom_bijung", "위험자산 편입비율 한도 및 비중 초과 시 처리",
              None, "위험자산 편입비율 한도 및 비중 초과 시 처리 방식"),
    TopicRule("연금수령한도", ("수령한도", "인출한도", "얼마까지 인출", "얼마나 인출",
                            "얼마까지 뽑", "한도",
                            "인출가능한 최대", "인출 가능한 최대",
                            "인출할 수 있는 최대", "받을 수 있는 최대",
                            "수령 가능한 최대", "수령가능한 최대",
    ...
    #    슬롯 자체가 안 만들어졌고, 나이(55세)를 줬는데도 원천징수세율이
    #    5.5%로 확정되지 못한 채 근거 문서의 "5.5~3.3%" 범위 서술만
    #    답변에 남았다.
    ...
    # ⚠️ 계산 슬롯은 calc_needs가 채워졌을 때만 만든다. 제도 설명만 묻는
    #    "DB형이 뭐예요?"까지 계산을 걸면 빈 계산 카드가 나간다.
    # ⚠️ '퇴직소득세' 계열 표현은 배제한다 — 세금은 별도 함수(퇴직소득세_계산)
    #    담당이고, 여기는 **적립액 자체**를 묻는 질의만 받는다.
    TopicRule("DB형_적립액", ("db형", "DB형", "확정급여", "확정급여형"),
              "db_jeokrip", "DB형 퇴직급여 적립액",
              "DB형_퇴직급여_계산", "DB형 퇴직급여 산정 방식",
              exclude=("퇴직소득세", "세금", "세율", "과세"),
              calc_needs=("avg_monthly_wage_manwon",)),
    TopicRule("DC형_적립액", ("dc형", "DC형", "확정기여", "확정기여형"),
              "dc_jeokrip", "DC형 적립 원금",
              "DC형_적립액_계산", "DC형 부담금 적립 방식",
              exclude=("퇴직소득세", "세금", "세율", "과세"),
              calc_needs=("salary_schedule",)),
    TopicRule("상품_비교", ("총보수", "보수가 낮", "수수료 비교", "어떤 클래스",
                         "비교해", "저렴한"),
              "chongbosu", "가입 가능한 클래스의 총보수 비교",
              "총보수_비교", "판매 클래스별 총보수"),
    # ── 상품 차원의 '차이' 질의 (F49 · 2026-09-06) ────────────────
    # ⚠️ 안내서 7페이지의 상품 비교 예시 질의
    #    "솔로몬 국공채 단기·중장기·장기, 뭐가 달라요? 안정적인 걸 원해요."
    #    가 위 규칙의 키워드 어디에도 안 걸렸다 — 사용자는 "총보수"·"비교해"
    #    라고 말하지 않고 **"뭐가 달라요"**라고 말한다. 그 결과 대주제 2의
    #    핵심 유형이 일반 폴백 슬롯으로 떨어져 무관한 문서(명예퇴직금)를
    #    근거로 답했고, 상품명을 3개나 정확히 줬는데도 "비교하시려는 상품의
    #    정확한 명칭을 확인해 주세요"라고 되물었다.
    # ⚠️ require_any로 **상품 차원임을 요구한다.** 이 게이트가 없으면
    #    "DC와 DB, 퇴직금이 정해지는 방식이 어떻게 다른가요?"(안내서 난이도
    #    '하'의 제도 질의)까지 상품 비교로 끌려간다.
    #    어휘는 DOMAIN_AREAS["상품"]을 **그대로 재사용**한다 — 같은 판단(이
    #    질의가 상품 차원인가)을 두 곳이 다른 기준으로 하면 반드시 어긋난다
    #    (CLAUDE.md "인용 기준과 매칭 기준을 어긋나게 두지 말 것").
    TopicRule("상품_비교",
              # ⚠️ '뭔'은 '뭐+ㄴ'의 축약이라 "차이가 뭐"가 "차이가 뭔가요"를
              #    **포함하지 않는다.** 한국어 부분문자열 매칭의 전형적인
              #    함정이라 두 형태를 모두 넣는다(CLAUDE.md 참조).
              ("뭐가 달라", "뭐가 다른", "무슨 차이", "어떤 차이", "차이점",
               "차이가 뭐", "차이가 뭔", "차이는 뭐", "차이는 뭔",
               "어떻게 달라", "어떻게 다른", "뭐가 좋", "뭔가 좋",
               "어느 게 나", "어느게 나"),
              "sangpum_chai", "상품 간 구조·위험등급·보수 차이",
              None, "상품별 위험등급·보수·수익률",
              require_any=_PRODUCT_DIMENSION),
    # ⚠️ 2026-09-05 외부 심사 리포트로 발견 — 이 규칙은 "이 펀드, 내가
    # 어떤 판매 클래스로 가입 가능한가"(상품 차원)를 노리고 만들었는데,
    # 키워드("가입할 수 있"·"가입 가능")가 "IRP는 아무나 가입할 수
    # 있나요?"처럼 **제도 차원**("근로소득자·자영업자 등 누가 이 제도에
    # 가입 가능한가")의 흔한 표현과 겹친다. 겹치면 난이도 '하'의 기초
    # 제도 질의가 "가입하려는 판매 클래스가 무엇인가요?"라는 엉뚱한
    # 역질문으로 빠진다(실사용 재현, 72초 지연 + 사실상 무응답).
    # 제도 차원 표현("아무나"·"누구나")이 있으면 이 규칙을 아예 배제해
    # 일반 사실 슬롯(_FALLBACK_SLOT)으로 넘긴다 — 검색이 실제 가입대상
    # 안내 문서를 그대로 찾아오게 두는 편이 억지 슬롯보다 낫다.
    TopicRule("가입자격", ("가입자격", "가입 자격", "가입할 수 있", "가입 가능"),
              "gaipjagyeok", "판매 클래스 가입자격",
              "판매클래스_적합성_판정", "종류별 가입자격",
              exclude=("아무나", "누구나", "가입 대상", "가입대상")),
    # ── 연금 외 수령 재원별 세금 (F48 · 2026-09-06) ───────────────
    # ⚠️ 함정 A9에 도메인 지식은 완전히 있는데 **계산기가 없었다.**
    #    "IRP에 퇴직금 1억, 세액공제 받은 납입액 3천만원, 운용수익
    #    2천만원이 있는데 지금 해지하면 세금이 얼마인가요"에 수치가 하나도
    #    안 나가고 함정 교정 문장만 실렸다(과제 안내 '기타(중도인출 등)' 유형).
    # ⚠️ calc_needs로 게이팅한다 — 재원 금액을 하나도 말하지 않은
    #    "중도인출 사유가 뭔가요?"까지 계산을 걸면 빈 계산 카드가 나간다.
    #    calc_needs는 OR 판정이므로 재원 하나만 말해도 그 부분은 답해 준다.
    # ⚠️ 아래 일반 "중도인출" 규칙보다 앞에 와야 한다(순서 계약).
    TopicRule("연금외수령_세금",
              ("해지하면", "해지 하면", "중도인출", "중도 인출", "중도해지",
               "연금 외 수령", "연금외 수령", "일시금으로", "깨면", "빼면"),
              "yeongeum_oe", "연금 외 수령 시 재원별 세금",
    ...
    TopicRule("중도인출", ("중도인출", "중도 인출", "중도해지", "깨면", "빼면", "꺼내"),
              ...
              "중도인출 사유와 적용 세율"),
    # ── 국민연금 계산 배선 (F48 · 2026-09-06) ────────────────────
    # ⚠️ 등록된 국민연금 함수 3종이 규칙 경로에서 **도달 불가**였다 —
    #    아래 "국민연금" 규칙의 calc_function이 None이라 CALC_REGISTRY에
    #    있어도 슬롯이 만들어지지 않았다. L1(HCX)이 직접 지정할 때만 돌았고,
    #    그게 바로 F41에서 근거 없이 지정돼 오라우팅을 일으킨 경로다.
    # ⚠️ calc_needs로 게이팅한다 — DB형/DC형과 같은 이유로, 제도 설명만
    #    묻는 "국민연금이 뭐예요?"까지 계산을 걸면 빈 계산 카드가 나간다.
    # ⚠️ 이 두 규칙은 아래 일반 "국민연금" 규칙보다 **앞**에 와야 한다
    #    (TOPIC_RULES는 앞의 규칙이 더 구체적인 주제라는 순서 계약이다).
    TopicRule("국민연금_본인부담금", ("본인부담", "본인 부담", "국민연금 보험료",
                                "국민연금보험료", "연금보험료 얼마"),
              "np_copay", "국민연금 월 본인부담금",
              "국민연금_본인부담금_계산", "국민연금 보험료 부담 구조",
              calc_needs=("monthly_income_manwon",)),
    TopicRule("출산크레딧", ("출산크레딧", "출산 크레딧"),
              "chulsan", "출산크레딧 인정개월수",
              "출산크레딧_인정개월_계산", "출산크레딧 인정 기준",
              calc_needs=("children_total",)),
    # ⚠️ 국민연금_수령액_계산은 **일부러 배선하지 않는다.** 필수 인자인
    #    소득대체율(r_irr)은 가입기간·전체가입자평균소득에 따라 달라져
    #    사용자가 말해 주는 값이 아니고 제공 자료로도 확정할 수 없다.
    #    배선하면 "국민연금 얼마 받나요"마다 '적용 소득대체율'을 되묻는
    #    슬롯이 생기는데, 그건 답이 아니라 알 수 없는 용어를 되묻는 것이다.
    #    검색 기반 일반 슬롯(아래 규칙)이 국민연금 제도 문서를 그대로
    #    찾아오게 두는 편이 낫다.

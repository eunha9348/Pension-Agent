...
# ⚠️ 긴 표기를 먼저 둘 것 — 정규식 교대(|)는 앞에서부터 맞으므로
#    "혼합채권형"이 "채권형"보다 뒤에 있으면 짧은 쪽이 먼저 먹는다.
#
# ⚠️ '채권혼합형·주식혼합형'은 **어순이 다른 같은 분류**다 (2026-09-05 추가).
#    법령 용어는 '혼합채권형' 계열이지만 금융투자협회 분류와 실물
#    투자설명서는 '채권혼합형'으로 쓰는 경우가 많다. 한쪽만 있으면
#    그 표기를 쓴 문서에서 상품분류가 통째로 사라진다(실측 커버리지
#    32.9%로 6축 중 두 번째로 낮은 축이다).
#    추가는 순수 가산이고 오탐 기전은 기존 항목과 동일하다(값이 갈리면
#    `_pick_one`이 비운다). 다만 **실물 발화 횟수는 서버에서
#    `python -m scripts.corpus_facts`로 확인할 것** — mock 코퍼스에는
#    이 축이 0회라 로컬에서는 측정이 불가능하다.
_ASSET_CLASSES: tuple[str, ...] = (
    "혼합자산", "혼합채권형", "혼합주식형", "채권혼합형", "주식혼합형",
    "재간접형", "재간접",
...
_ASSET_PATTERNS: tuple[tuple[str, re.Pattern], ...] = (
    ...
    #
    # ⚠️ 줄바꿈을 넘도록 `[^\n]`이 아니라 `[^【]`을 쓴다 (2026-09-05).
    #    실물 투자설명서의 지배적 형태는 제목과 값이 **다른 줄**에 있다:
    #        【집합투자기구의 종류】
    #        증권집합투자기구(채권혼합형)
    #    `[^\n]`이면 이 형태를 통째로 놓친다 — 같은 문서에서 위험등급은
    #    잡히는데 상품분류만 빠지던 원인이다(커버리지 63.3% vs 32.9%).
    #    대신 **다음 절 제목(【)에서 멈춘다** — 40자 안에서 절을 넘어가
    #    다른 절의 분류어를 긁으면 그게 오탐이다.
    ("종류_표제", re.compile(
        r'(?:집합투자기구|투자신탁|펀드)\s*의?\s*종류[^【]{0,40}?'
        rf'({_ASSET_ALT})')),
    # "상품분류 | 채권형" / "자산유형: 주식형"
    #
    # ⚠️ 이쪽은 **줄 안에 머문다** — 위의 종류_표제와 달리 줄바꿈을 넘기면
    #    안 된다. 이 표제어들(특히 '투자대상')은 제목 뒤에 분류값이 아니라
    #    **산문**이 오는 절의 머리말이기도 하다. 넘기게 했더니
    #    "【투자대상】\n이 펀드는 주식형 자산에 투자하지 않습니다"가
    #    상품분류=주식형으로 잡혔다(부정문인데 정반대로 확정한다).
    #    종류_표제는 '집합투자기구의 종류'라는 값 필드라 다음 줄이
    #    곧 값이지만, 여기는 그 보장이 없다.
    ("분류_표제", re.compile(
...
# ⚠️ '운용실적' 단독은 넣지 않는다. 실측(재인덱싱본)에서 표준 위험등급
#    경고문 "펀드의 위험 등급은 운용실적, 시장 상황 등에 따라 변경될 수
#    있다"에 걸려, **위험등급 문장 전체를 '과거 운용실적 표'로 잘못
#    인용했다.** 표 헤더는 실측에서 확인된 "기구 수 운용규모 최근1년
#    최근2년" 형태처럼 기간을 반복 나열하는 것이 특징이므로, 그 형태와
#    명시적 수익률 표현만 남긴다.
_RETURN_TABLE_HEADER = re.compile(
    r'^[^\n]{0,40}(?:최근\s*\d\s*년[^\n]{0,30}최근\s*\d\s*년|'
    r'기간별\s*수익률|연평균\s*수익률표|운용실적표|'
    r'수익률\s*현황)[^\n]{0,60}$', re.M)
...
def _quote_table(text: str, header: re.Pattern, axis: str, label: str,
    ...
    # ⚠️ **다음 절 제목에서 멈춘다.** 줄 수만으로 자르면 표가 짧을 때
    #    뒤 절이 딸려 온다 — 실측 재현(2026-09-05): 총보수표 스니펫에
    #    "【집합투자기구의 규모】 설정원본 1,234억원"이 함께 잘려 들어왔다.
    #    이 스니펫은 ① 생성 프롬프트에 "이 값만 사용 가능"으로 실리고
    #    ② `fact_snippets`를 거쳐 **수치 검증 허용 집합**에 들어간다.
    #    즉 다른 절의 수치가 총보수인 것처럼 근거를 얻는다 —
    #    "근거 뭉치 전체를 허용 집합에 넣지 않는다"(F8)와 같은 결함이다.
    #    투자설명서는 【제목】으로 절이 갈리므로 그 경계에서 자른다
    #    (머리말 자신이 【…】일 수 있으므로 첫 줄은 제외한다).
...
_AUM_PATTERNS: tuple[tuple[str, re.Pattern], ...] = (
    # '설정원본'은 투자설명서가 펀드 규모를 적을 때 쓰는 표준 표기인데
    # 빠져 있었다 (2026-09-05 추가). 시장잔고는 실측 커버리지 8.2%로 6축 중
    # 가장 낮은 축이라 표기 하나가 빠지면 영향이 크다. 아래 _AUM_BLOCKLIST가
    # 규제 기준선("설정액 50억원 미만인 소규모…")을 그대로 막아 주므로
    # 오탐 기전은 기존 표기와 동일하다.
    # ⚠️ 실물 발화 횟수는 서버에서 `python -m scripts.corpus_facts`로 확인할 것.
    ("표제_잔고", re.compile(
        r'(?:시장\s*잔고|설정\s*액|설정\s*원본|순자산\s*총?액|'
        r'운용\s*규모|펀드\s*규모)'
...
#
# ━━ 실측 결과 (158문서, 2026-09-04) ━━
# 이 축은 **0건**이다. 그런데 패턴 실패가 아니라 **데이터 부재**다.
# '약정이율'은 25회·17문서에 있지만 전부 위험 고지문이고 수치가 없다:
#     "중도해지 시 약정이율의 축소 적용 등 불이익이 발생"
#     "약정이율 축소 적용 등으로 당초 기대했던 수익보다 적어질 위험"
# 아래 '중도해지' 차단어가 정확히 저 문장을 막았다 — 안 막았다면 엉뚱한
# 값이 그 상품의 금리로 나갔을 것이다. 즉 0건이 올바른 동작이다.
# (공시이율·적용금리는 코퍼스 등장 0회. 이 코퍼스에 원리금보장형 상품
#  문서가 없다 — 158건이 실적배당형 투자설명서와 제도 안내다.)
# 패턴은 그대로 둔다. 평가용 코퍼스가 다르거나 원리금보장형이 추가되면
# 그때 동작하고, 지금은 아무것도 잘못 뽑지 않는다.
...
_MIN_FACT_LABEL_CHARS = 3
# 표 축(총보수표·실적표)의 label은 값이 아니라 자리표시자다("총보수 표(원문 인용)").
# 답변에 그대로 나타날 일이 없으므로 반영 판정에서 제외한다 —
# 이 축들의 내용은 snippet에 있고, 답변이 표를 통째로 인용하지는 않는다.
_FACT_LABEL_PLACEHOLDER = "원문 인용"


def facts_reflected_in_answer(answer: str, facts: list[dict]) -> dict[str, str]:
    """답변이 실제로 반영한 팩트의 **근거 문서**를 doc_id -> 사유로 돌려준다.

    ━━ 왜 필요한가 (2026-09-05 배선 테스트로 드러남) ━━
    팩트 블록은 `evidence`(검색 결과)의 doc_id로 만들어 생성 프롬프트에
    실린다. 그런데 `retrieved_context`는 `_used_evidence`(슬롯에 매핑된
    근거)로만 만들어진다. 두 집합은 같지 않다 — 슬롯 매핑이 하나도
    성립하지 않으면 인용은 비고 `retrieved_context`에는 "근거 문서 없음"이
    찍히는데, 그 사이에도 팩트 블록은 생성기에 위험등급·총보수·수익률을
    그대로 넘긴다. 그러면 **답변에는 4등급이 있는데 그 근거 문서는
    retrieved_context 어디에도 없는** 상태가 된다.

    평가 API 스펙이 `retrieved_context`를 "답변 생성에 참고한 검색 문서"로
    정의하므로 이건 스펙 위반이다. 함정 근거 문서를 `_addressed_trap_docs`가
    같은 이유로 인용에 되살리는 것과 정확히 같은 사례다.

    판정은 결정론적이다 — 팩트 표기(label)가 답변에 **글자 그대로**
    있을 때만 반영으로 본다(공백만 정규화). 값을 바꿔 쓰면 잡지 않는다.

    ⚠️ 이 판정은 **인용을 더하는 방향으로만** 쓴다. 심각도를 올리거나
       재생성을 부르지 않으므로, 여기서의 오탐은 "근거에 이미 오른 문서가
       retrieved_context에 함께 실리는 것"에 그친다. 반대로 미탐은
       근거 없는 수치를 내보내는 결함이 된다.
    """
    ...
    for f in facts:
        ...
        if not doc_id or doc_id in out:
            continue
        for axis, label, _snippet in _fact_lines(f):
            if _FACT_LABEL_PLACEHOLDER in (label or ""):
                continue
            key = re.sub(r"\s+", "", label or "")
            if len(key) < _MIN_FACT_LABEL_CHARS:
                continue
            if key in norm_answer:
                out[doc_id] = f"{axis}({label})"
                break
    return out

...
# ⚠️ '노후 대비'·'노후대비'만으로는 부족했다(2026-09-06, UI-020 실측) —
#    "부동산 20억, 군인연금 월 400인데 노후를 어떻게 대비해야할까"는 '노후'와
#    '대비' 사이에 조사('를')가 끼어 두 리터럴 중 무엇과도 안 걸렸다.
#    '어떻게 해야'·'어떻게 준비'와 같은 계열로 '어떻게 대비'를 추가해
#    조사가 끼는 흔한 어순을 잡는다. '노후 준비'도 같은 이유로 추가한다.
_ADVISORY_SIGNALS: tuple[str, ...] = (
    "계획", "설계", "어떻게 해야", "어떡해", "어떻게 준비", "어떻게 대비",
    "노후 대비", "노후대비", "노후 준비", "노후준비",
    "추천", "조언", "상담", "괜찮을까",
...
def classify_route(question: str,
    ...
    2. 계좌유형·클래스가 있다       → GENERAL (기존 자격·비교 로직이 돈다)
    3. 상담 신호 + 개인 서술        → ADVISORY (계산 조건 없이 계산함수만
                                      지정된 경우보다 이 신호를 신뢰한다 — 아래 참조)
    4. 계산함수가 지정됐다          → GENERAL
    5. 계산 앵커가 전혀 없다        → ADVISORY (물을 수치 자체가 없다)
    6. 그 밖                        → GENERAL (기본값은 기존 경로)

    ⚠️ **계산함수 지정(2번 → 4번으로 순서 이동, 2026-09-06)**만으로는
    상담 신호를 이기지 못하게 했다. 실측(UI-020) — "부동산 20억, 군인연금
    월 400인데 노후를 어떻게 대비해야 할까"에서 L1이 아무 근거 조건 없이
    '국민연금_수령액_계산'을 slot에 지정했다(존재하는 등록 함수라
    supervise_plan의 화이트리스트를 그대로 통과한다). has_calc_cond는
    False였는데(뒷받침하는 수치가 하나도 없음) has_calc_slot만으로
    GENERAL이 강제됐고, 그 계산은 필수 인자 부족으로 곧장 실패해
    "확인이 필요합니다"라는 무의미한 답변만 남았다 — 정작 질문은
    "노후 대비"라는 전형적 ADVISORY 문구였는데도.
    supervise_plan은 **등록 여부**만 검증하지 **관련성**은 검증하지
    않는다(그건 계획감사의 역할이 아니다). 반면 개인 서술+상담 신호는
    질의 원문에서 직접 뽑은, 계산 조건보다 훨씬 조작하기 어려운 신호다.
    그래서 "계산 조건 없이 계산함수만 있는" 경우에는 이 신호가 우선하도록
    순서를 바꿨다 — has_calc_cond(진짜 수치)가 있으면 여전히 1번에서
    최우선으로 GENERAL이 확정되므로, 실제로 계산할 수치가 있는 질의는
    영향받지 않는다.
    """
    q = question or ""
    cond = conditions or {}
    ...
    if advisory_signal and personal:
        return RouteDecision(
            ...
            f"계좌유형·클래스·수치가 없어 계산으로 답할 수 없음"
            + (" (계산함수가 지정됐으나 뒷받침하는 수치가 없어 신뢰하지 않음)"
               if has_calc_slot else ""), sig)
    if has_calc_slot:

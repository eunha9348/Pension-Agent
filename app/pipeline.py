...
#
# 2026-09-02 — 환경변수로 빼고, 상한 60초에 맞춰 55초로 올린다.
#   실측에서 REVISE가 떴는데 `L6_재생성_생략`으로 빠지는 경우가 나왔다.
#   이 예산은 **평가 측 타임아웃과의 트레이드오프**다 — 올리면 재생성
#   기회가 생기고, 너무 올리면 평가가 먼저 끊는다. 운영자가 확인해 준
#   상한은 60초이므로, 마지막 단계가 그 안에서 끝나도록 55초를 기본으로
#   둔다(아래 "왜 55인가" 참조). 배포에서 재조정할 수 있게 값은
#   PIPELINE_BUDGET_SEC로 노출한다.
#
...
TOTAL_BUDGET_SEC = _budget_sec("PIPELINE_BUDGET_SEC", 55.0)
...
#
# 왜 55인가 (운영자 확인 상한 60초 기준):
#   예산이 정직하면 **어떤 단계든 늦어도 t≈TOTAL에 끝난다** — 게이트가
#   `remaining >= need`일 때만 열리기 때문이다. 그 뒤는 전부 결정론적이라
#   1초 미만이다. 따라서 55초는 60초 상한 안에서 닫히고 4초 여유가 남는다.
#   상한과 총 예산을 같은 값으로 두면 안 된다 — 게이트는 중단 장치가
#   아니라서 마지막 단계가 통째로 상한 밖에서 끝난다.
#
#   ⚠️ 전 구간(L1 + L5' + L6 + 재생성 + 구제 재생성 = HCX 7회 ≈ 56초)은
#      60초 안에 들어가지 않는다. 이건 예산으로 풀 수 있는 문제가 아니라
#      상한이 정한 사실이다. 그래서 우선순위가 앞선 재생성(BUDGET_REGEN)이
#      먼저 열리고, 구제 재생성은 앞 단계들이 빨리 끝났을 때만 열린다.
#      열리지 못하면 `SubAgent_구제_생략`에 남은 시간과 함께 기록된다 —
#      조용히 사라지지 않으므로 나중에 값을 조정할 근거가 된다.
#   ⚠️ 호출 하나가 타임아웃(15초)→재시도를 타면 이 계산은 깨진다.

...
class PresenceResult:
    ...
    def as_trace(self) -> str:
        if self.required_count == 0:
            # ⚠️ 예전 문구는 "답변에 실려야 할 계산 결과가 없음"이었다. 이건
            #    "계산은 했는데 답변에 없다"는 **위반 보고**처럼 읽히는데,
            #    실제 뜻은 "계산 단계 자체가 돌지 않아 요구할 값이 없다"다.
            #    같은 trace에 APPROVE가 함께 찍히니 외부 심사에서 '자기 진단과
            #    출력이 어긋난 사례'로 지목됐다(2026-09-06 UI 평가 1번).
            #    판정 로직은 그대로고, 진행 로그가 사실을 정확히 말하게 고쳤다.
            return "계산값 표기 — 검사 대상 없음 (계산 단계가 실행되지 않음)"
        if self.passed:

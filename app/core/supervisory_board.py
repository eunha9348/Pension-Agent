...
5. 의도 충족 — 형식적으로 답했더라도 질문자가 실제로 알고자 한 것을
   다뤘는가. 되묻는 항목이 정말 결정적인가, 답할 수 있는데 미루는가.
6. 답변 태도 — 과장·불안 조장·지나친 단정·무책임한 회피가 없는가.

━━ 판정 기준 ━━
· 1~4(정합성)에서 어긋남을 찾으면 REVISE 이상으로 판정하십시오.
  숫자가 맞아도 논리가 어긋나면 그 답변은 틀린 답변입니다.
· 5~6은 정합성이 무너지지 않았다면 지적하되 판정은 신중히 하십시오.
...

{
  ...
  "findings": [
    {"code": "위 6개 항목 중 하나", "detail": "무엇이 무엇과 어긋나는지",
     "directive": "구체적 시정 지시"}
  ],

...
def audit_anomaly(calc_results: list[dict],
    ...
    for r in _flatten_variants(calc_results):
        ...
        # ⚠️ 위 두 판정은 "한도를 넘었다"는 사실만 요구한다. 그런데 연금저축
        #    단독 한도 초과분은 **버려지는 게 아니라 IRP로 옮기면 살릴 수
        #    있다** — "얼마까지 받나"가 아니라 "어떻게 넣어야 최대로
        #    받는가"를 묻는 질의에서 이 재배치 여지를 놓치면, 계산은 맞아도
        #    사용자가 손해 보는 분배를 그대로 둔 채 답변이 끝난다
        #    (2026-09-07 실측 — 사용자가 손해 보는 분배를 말했는데도
        #    바로잡지 못함). 계산함수가 이미 재배치 시 추가로 받을 수
        #    있는 공제액을 냈으면, 그 사실을 **적극적으로 권고**했는지를
        #    REVISE 대상으로 삼는다 — 단순 명시가 아니라 행동을 권해야
        #    하므로 다른 한도 판정보다 요구 수위가 높다.
        if r.get("IsReallocatable") is True:
            findings.append(Finding(
                "이상치", "REALLOCATION_MISSED", Verdict.REVISE,
                "연금저축 단독 한도 초과분을 IRP로 옮기면 추가 공제를 받을 "
                "수 있는 상태가 계산에 반영됨",
                "연금저축 한도를 초과한 금액 중 일부를 IRP로 옮기면 추가로 "
                "세액공제를 받을 수 있다는 점을 구체적인 금액과 함께 "
                "적극적으로 권고할 것 — 단순히 '초과분은 공제 대상이 "
                "아니다'라고만 쓰고 넘어가면 안 됨",
            ))
...
1. 수치–서술 정합 — 제시된 숫자와 그것을 설명하는 문장이 서로 맞는가.
   ...
   **위험등급 방향도 같은 자리에서 보십시오** — 1등급이 가장 높은 위험이고
   6등급이 가장 낮은 위험입니다(숫자가 작을수록 위험함). 등급 번호는 맞게
   인용했는데 그 번호를 "안전하다"·"위험하다"·"중간 정도"로 잘못된 방향의
   말로 풀어 쓰면 숫자–서술 불일치와 같은 결함입니다.
   (예: 2등급을 "중간 정도의 위험"이라고 서술 — 2등급은 표준 6단계에서
   최고위험(1등급) 바로 다음으로 높은 위험 구간이므로 틀렸습니다)
   **조건부 값의 서술 순서도 같은 자리에서 보십시오** — 소득 구간처럼
   서로 배타적인 조건에 따라 갈리는 값을, 하나를 먼저 확정적으로 말한
   뒤 다른 조건의 값을 예외처럼 덧붙이면 두 값이 함께 적용되는 듯한
   잘못된 인상을 줍니다. 실제로는 조건에 따라 하나만 적용됩니다.
   (예: "600만원 이내 13.2%가 적용됩니다. 다만 소득이 낮으면 16.5%도
   가능합니다" — 조건(소득 구간)이 먼저 나와야 할 자리에서 값을 먼저
   확정하고 조건을 예외처럼 덧붙였으므로 정합성이 어긋난 서술입니다)
...
def build_llm_audit_payload(answer: str,
    ...
    if law_articles:
        parts.append(
            ...
            "저촉이 아닙니다. **제공 문서(근거 문서·계산 결과)가 최종 근거이고 "
            "조문은 판정 보조 자료입니다.** 답변이 제공 문서 내용을 그대로 "
            "옮긴 것이라면(지어낸 서술이 아니라면), 조문과 표현이 다르다는 "
            "이유만으로 저촉으로 내지 마십시오 — 제공 문서가 더 구체적이거나 "
            "다른 시점 기준일 수 있습니다. 숫자 자체가 명백히 틀린 경우"
            "(예: 개정 전 한도·세율을 현행처럼 서술)에만 저촉으로 내십시오.")

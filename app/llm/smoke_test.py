...
app/pipeline.py의 타임아웃 예산은 2026-08-18 실측치로 이미 갱신돼 있습니다.
모델이나 서버를 바꿨다면 이 스크립트로 다시 재고, 출력되는 제안값으로
BUDGET_* 상수를 갱신하십시오.

⚠️ 여기서 재는 것은 **짧은 프로브 문장**의 지연입니다. 실제 L5'/L6은 근거
   문서가 실린 3,000토큰짜리 프롬프트라 5~8초가 정상입니다 — 이 스크립트
   결과(1초 미만)만 보고 타임아웃을 조이면 정상 호출이 튕겨 나갑니다.
"""

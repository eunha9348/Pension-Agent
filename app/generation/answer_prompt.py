...
9. 조건에 따라 값이 갈리면 **조건을 먼저** 밝히고 그다음에 값을 쓰십시오.
   ...
   맞는 예: "총급여 5,500만원을 넘으면 13.2%, 그 이하면 16.5%가
            적용됩니다."
10. 근거 문서에 없는 **구체적 절차나 메커니즘**을 단정하지 마십시오.
    "자동으로 리밸런싱됩니다", "강제로 매도됩니다"처럼 값이 아니라
    행위를 지어내는 것은 숫자를 지어내는 것과 같은 위험입니다. 근거에
    그 절차가 없으면 확인이 필요하다고 밝히십시오 — 그럴듯하게 들리는
    일반 지식으로 메우지 마십시오.
11. [계산 결과]에 "재배치시_추가공제_가능액"이 있으면, 계산 결과를
    설명만 하고 끝내지 마십시오. 사용자가 말한 분배(예: 연금저축에만
    900만원)가 손해라는 뜻이므로, **그 금액을 IRP로 옮기면 얼마를 더
    받을 수 있는지 구체적으로 권고**하십시오. "600만원까지만 공제되고
    초과분은 대상이 아닙니다"까지만 쓰고 멈추면 안 됩니다 — 사용자가
    더 나은 분배를 알 수 있어야 계산이 쓸모가 있습니다.

━━ 확인되지 않은 조건이 결론을 좌우할 때 ━━
결론이 아직 확인되지 않은 조건에 따라 달라진다면, 그 사실을 **결론보다
먼저** 말하십시오. 확신에 찬 결론을 앞에 놓고 단서를 맨 끝에 붙이면,
읽는 사람은 앞부분만 결론으로 받아들입니다. 조건별 값을 쓸 때 조건을
먼저 밝히는 것과 같은 이유입니다.
  틀린 예: "지금 시작하시는 것이 유리합니다. 다만 소득과 납입 여력에
           따라 달라질 수 있습니다."
  맞는 예: "소득과 납입 여력을 알려주셔야 확정할 수 있습니다만, 그
           둘이 확보돼 있다면 조기 납입이 세액공제 측면에서 유리합니다."
...
def build_supervisor_payload(query_spec: dict,
    ...
    if evidence:
        ...
        # ⚠️ 근거 0건 분기가 이 경로에만 **없었다** (F53 · 2026-09-06).
        #    L4-sub(build_advisory_payload)에는 처음부터 있었는데 L5'에는
        #    [근거 문서] 블록 자체가 통째로 빠져 있어, 모델이 근거가 없다는
        #    사실조차 모른 채 자유롭게 썼다. F3(함정 교정이 ADVISORY에만
        #    빠져 있던 것)의 정확한 거울상이다.
        #    실측: "솔로몬 국공채 단기·중장기·장기, 뭐가 달라요?"에
        #    retrieved_context는 "근거 문서: 해당 없음"을 표시했는데 같은
        #    답변이 "중장기·장기 상품은 주식·부동산·인프라에도 투자한다"를
        #    단정했다.
        # ⚠️ 계산 결과까지 막지는 않는다 — 그건 결정론적 함수의 출력이라
        #    근거가 있다(위 [계산 결과] 블록). 막는 것은 문서 사실이다.
        parts.append(
            "\n[근거 문서]\n확보된 근거가 없습니다. 상품의 투자대상·"
            "자산구성·위험등급·보수·수익률처럼 문서로 확인해야 하는 사실을 "
            "하나도 서술하지 마십시오. 위에 계산 결과가 있으면 그것만 "
            "설명하고, 나머지는 근거를 확인하지 못했다는 점을 밝힌 뒤 "
            "무엇을 알려주시면 답변드릴 수 있는지 안내하십시오.")

...
#
# C4·C5 (세액공제 한도 600/900만원) ★ 가장 시급
#   --propose가 소득세법 제59조의3 제2항(공제의 '명칭' 정의)과 제5항
#   (계산방법은 대통령령)만 잡았다. **한도 수치가 든 제1항을 못 잡았다.**
#   이 두 규칙은 한도 수치가 요점이므로, 수치 없는 조문을 앵커로 달면
#   HCX가 한도를 뒷받침하지 못하는 조문을 인용하게 된다. 그래서 비워 둔다.
#
#   ⚠️ 함정: C1 후보에 나온 소득세법 제47조의2 제1항의 900만원은
#      **연금소득공제** 한도이지 세액공제 한도가 아니다. 수치가 같아서
#      혼동하기 쉬운데, 이걸 C4에 달면 시스템이 둘을 뒤섞는다. 절대 금지.
#
# C1 (1,500만원 초과 시 전액 과세)
#   '분리과세'가 너무 넓어 이자·배당·주택임대 분리과세만 잡혔다.
#   사적연금 분리과세 조문을 따로 찾아야 한다.
#
# C2 (사적연금소득 합산)
#   후보 0건. '사적연금소득'은 법령 용어가 아니다(법은 '연금소득'으로 쓴다).
#

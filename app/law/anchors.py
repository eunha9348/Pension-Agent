...
ANCHORS: dict[str, tuple[str, ...]] = {
    ...
    # 1,500만원 기준과 그 효과. 제14조 제3항 제9호 다목이 핵심이다 —
    #   "가목 및 나목 외의 연금소득의 합계액이 연 1천500만원 이하인 경우
    #    그 연금소득"
    # 금액 기준은 '합계액'에 걸리고 효과는 '그 연금소득'(전체)에 미친다.
    # 초과분만 떼어내는 구조가 아니라는 것이 이 문장에서 직접 읽힌다.
    # 제64조의4는 초과 시의 선택(종합소득 결정세액 vs 연금소득 15% + 나머지)을
    # 정하므로 '선택하게 된다'는 서술의 근거가 된다.
    "C1": (
        "소득세법 제14조 제3항",
        "소득세법 제64조의4",
    ),

    # 1,500만원 산정에서 빠지는 것들. 같은 제9호가 두 겹으로 정한다 —
    #   · 본문이 "제20조의3제1항제2호 및 제3호에 따른 연금소득"만 대상으로
    #     삼아 공적연금소득(제1호)이 애초에 빠진다
    #   · 다목이 "가목 및 나목 **외의**" 연금소득만 합산하므로, 가목의
    #     "의료목적, 천재지변이나 그 밖에 부득이한 사유 등 … 인출하는
    #     연금소득"이 산정에서 제외된다
    "C2": ("소득세법 제14조 제3항",),

    # 분리과세 세율 15%. 제64조의4 가목의 "100분의 15"가 근거다.
    # ⚠️ 16.5%는 여기에 지방소득세 10%를 더한 값인데, 그 근거는 지방세법이라
    #    수집 범위 밖이다. 즉 이 앵커는 15% 쪽만 뒷받침한다. 인용 검증이
    #    수집본에 없는 조문 인용을 막으므로 과잉 인용 위험은 없다.
    "C3": ("소득세법 제64조의4",),
...
#
# ⚠️ C1·C2가 한때 "수집본에 1,500만원이 없다"로 보였던 이유를 남겨 둔다.
#    실제 원인은 표기 문제가 아니라 **크롤러가 각 호를 버리고 있었던 것**이다
#    (crawler._clause_text 주석 참고). 호를 담아 재수집한 뒤에야 제14조
#    제3항 제9호 다목의 "연 1천500만원"이 드러났다. 앞으로도 "법령에 없다"는
#    결론을 내리기 전에 수집 단계부터 의심할 것.
#
...
def main(argv: list[str] | None = None) -> int:
    ...
    # --grep/--ref 는 뒤따르는 값을 모두 받는다(nargs="+"). 그래서 뒤에 붙인
    # 옵션이 인식되지 않으면 **조용히 검색어로 삼켜진다.** 실제로 자판이
    # '--show'를 em대시 '—show'로 바꿔 넣는 바람에 검색어가 4개가 됐고
    # 결과는 0건이었다(2026-08-28). 조용한 오작동보다 즉시 실패가 낫다.
    if dashy := [t for t in (a.grep or []) + (a.ref or [])
                 if t.startswith(("-", "–", "—", "―"))]:
        print(f"검색어에 옵션처럼 보이는 값이 섞였습니다: {dashy}")
        print("  --grep/--ref 는 뒤따르는 값을 전부 받습니다. 옵션은 앞에 두십시오:")
        print("      --show 900 --grep 연금소득 1천500만원")
        print("  자판이 붙임표 두 개(--)를 대시 하나(—)로 바꾸기도 합니다. 확인하십시오.")
        return 2

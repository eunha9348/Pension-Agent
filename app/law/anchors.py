...
SEARCH_TERMS: dict[str, tuple[str, ...]] = {
    ...
    "C6": ("이연퇴직소득",),
    "D1": ("연금수령",),
    # 제도 전환의 방향별 가부. '퇴직연금제도 + 변경'은 정의·부담금 조문만
    # 잡았다. 법문은 전환을 "다른 퇴직연금제도로 변경"으로 쓰므로 그 구절을
    # 통째로 노린다. 호까지 담아 재수집했으므로 이번엔 잡힐 가능성이 높다.
    "E5": ("다른 퇴직연금제도로 변경",),
    "E5_대안": ("퇴직급여제도의 변경",),
}
...
ANCHORS: dict[str, tuple[str, ...]] = {
    ...
    # IRP 인출은 '법정 사유'에 걸린다는 쪽의 근거. 두 조문이 이어진다 —
    #   제24조 제5항: "개인형퇴직연금제도의 급여의 종류별 수급요건 및
    #                  중도인출에 관하여는 대통령령으로 정한다"
    #   시행령 제18조 제2항: "가입자가 다음 각 호에 해당하는 경우에는
    #                  법 제24조제5항에 따라 … 중도인출할 수 있다"
    # '다음 각 호에 해당하는 경우에는'이 곧 사유 제한이다.
    #
    # ⚠️ A2는 IRP와 연금저축의 **차이**가 요점인데, "연금저축은 사유 제한이
    #    없다"는 쪽은 규정의 **부재**라서 조문으로 증명할 수 없다. 근퇴법이
    #    연금저축에 적용되지 않는다는 적용범위 조문을 찾으면 보강할 것.
    #    그때까지 이 앵커는 IRP 쪽만 뒷받침한다.
    #
    # 제22조(DC 중도인출)는 넣지 않는다 — A2가 다루는 것은 IRP다.
    "A2": (
        "근로자퇴직급여 보장법 제24조 제5항",
        "근로자퇴직급여 보장법 시행령 제18조 제2항",
    ),
...
#
# E5 (제도 전환 방향 — DB→DC는 되지만 DC→DB는 안 된다)
#
#   ★ 두 차례 검색 모두 0건이다(2026-08-28, 실수집본 7,426건).
#        '퇴직연금제도' + '변경'      → 정의·부담금 조문만
#        '다른 퇴직연금제도로 변경'    → 0건
#
#   여기서 표기를 더 찍어보는 대신 가설을 세워 둔다: **방향별 가부가 애초에
#   법조문에 없을 수 있다.** 근퇴법은 제도의 설정·변경 '절차'(근로자 과반수
#   동의 등)를 정하지, "DC에서 DB로 되돌릴 수 없다"를 조문으로 금지하지는
#   않는 것으로 보인다. 그렇다면 이 방향성은 적립금 귀속 구조에서 따라오는
#   해석이지 법문이 아니며, 그 경우 이 규칙에 붙일 조문은 존재하지 않는다.
#
#   ⚠️ 이 가설은 **확인되지 않았다.** 확인 전까지는 비워 두는 것이 맞다.
#      전환을 언급만 한 조문을 달면, 방향별 가부를 뒷받침하지 못하는 근거를
#      HCX가 인용하게 된다 — 수치 없는 조문을 C4에 달지 않은 것과 같은 이유다.
#      앵커가 없어도 E5의 결정론적 감지는 그대로 동작하므로 손해는 없다.
